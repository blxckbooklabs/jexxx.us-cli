...
- **Prompt overlay editor shortcuts + cursor (July 2026 fix):** Non-secret prompts now support full Emacs-style editor shortcuts with cursor positioning and selection highlighting. Secret mode (API keys) keeps the existing behavior: dots for masking, Shift+P for paste. Cursor and selection state are tracked via `promptCursorPos` and `promptSelectionStart` React state.
