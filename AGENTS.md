...
- (None)
- **Picker filter selection desync (July 2026 fix):** The Dye picker rendered filtered rows from `typedQuery` but Enter/arrow handlers resolved picks from stale `pickerState.filterQuery` (often empty). Typing `xai` could highlight "+ xAI" while Enter still selected index 0 of the full list (`+ OpenCode Zen`), sending users through the wrong provider setup and Zen model list. Fixed with shared `picker-filter.ts` (`resolvePickerSelection`) and consistent `typedQuery` use in keyboard handlers; filter edits reset `selectedIndex` to 0.
...
- **Picker filter editor shortcuts + paste (July 2026 fix):** The picker filter box now supports full Emacs-style keyboard shortcuts matching the main chat input (`InputView.tsx`). Paste works via `usePaste` when filter is focused. Shortcuts include: Ctrl/Meta+Left/Right (word jump), Ctrl/Meta+Shift+Left/Right (word select), Shift+Left/Right (char select), Home/End (line start/end), Ctrl+A/E/U/K/W (line editing), Ctrl/Meta+Backspace (delete word), Delete (delete char/selection). Cursor position is now rendered mid-text with a `█` block when focused, and active selection is highlighted in pink inverse. Filter cursor and selection state are tracked via `filterCursorPos` and `filterSelectionStart` React state.
- **Search overlay editor shortcuts + cursor (July 2026 fix):** The transcript search overlay (`/`) now supports the same Emacs-style editor shortcuts. The filter renders a cursor at the exact position with `█` block, and supports selection highlighting. Cursor and selection state are tracked via `searchCursorPos` and `searchSelectionStart` React state.
- **Prompt overlay editor shortcuts + cursor (July 2026 fix):** Non-secret prompts now support full Emacs-style editor shortcuts with cursor positioning and selection highlighting. Secret mode (API keys) keeps dots for masking and clipboard paste via `usePaste`, Cmd/Ctrl+V, or standalone `P` (reads `pbpaste` on macOS). Cursor and selection state are tracked via `promptCursorPos` and `promptSelectionStart` React state.
- **Transcript scrolling broken in Dye TUI (July 2026 fix):** The Dye-based TUI had empty stub scroll callbacks — `scrollUp()`, `scrollDown()`, etc. were no-ops. Fixed by adding `scrollOffset`, `scrollUp()`, `scrollDown()`, `scrollPageUp()`, `scrollPageDown()`, `scrollToTop()`, and `scrollToBottom()` methods to `MessageStore`. `DyeApp.tsx` now uses `store.scrollOffset` directly, and `DyeAdapter` properly invokes the store methods. Scroll controls work via: `↑↓` (line-by-line when no thinking focus), `PgUp`/`PgDn` (half-viewport), `Home`/`End` (jump), `Shift+↑↓` (line scroll from input focus).
- **Ollama Cloud uses wrong adapter (openai instead of ollama) (July 2026 fix):** Ollama Cloud had `adapter: "openai"` in catalog.ts, causing `listModelsForProvider()` to hit its `/v1/models` endpoint (OpenAI-compatible discovery path) instead of the correct `/api/tags` endpoint. The `/v1/models` endpoint apparently served the wrong catalog — users selecting Ollama Cloud were shown OpenCode Zen model IDs. Changing `adapter` to `"ollama"` routes model discovery through `listOllamaModels()` which uses `/api/tags` (the correct Ollama API endpoint). Runtime behavior is unchanged — `createOllamaProvider()` wraps `createOpenAIProvider` anyway.
- **Embedded desktop clipboard (July 2026 fix):** When `JEXXXUS_EMBEDDED=1` (jexxx.us-desktop PTY),
  `copyToClipboard()` in `tui-snapshot.ts` skips `pbcopy`/`xclip` and emits OSC 52 only — native
  clipboard commands hang or target the wrong clipboard inside `ELECTRON_RUN_AS_NODE`. Dye
  `useSelection().copy()` already prefers OSC 52 with ST (`ESC \`) termination; the desktop app
  intercepts both ST and BEL terminators in `src/shared/terminal-osc52.ts` and writes via
  `electron.clipboard`.
- **Prompt overlay paste broken on macOS (July 2026 fix):** On macOS, terminal emulators intercept Cmd+V at the OS level — blessed/Dye often never receive paste events. Additionally, blessed 0.1.81 doesn't handle bracketed paste mode (`\x1b[?2004h`), which modern terminals enable by default. Fixed via: (1) disable bracketed paste on overlay open (`\x1b[?2004l`), re-enable on close; (2) `pasteInFlight` dedup; (3) shared `readClipboardRobust()` (`/usr/bin/pbpaste` on darwin); (4) standalone `P` paste fallback via `isSecretPromptPasteKey()` in `secret-prompt-input.ts`. **Regression (July 2026):** Dye secret-prompt handlers were accidentally nested inside the non-secret `useInput` block — API key entry ignored Enter/Esc/typing/paste. Fixed by a dedicated `promptState?.options.secret` branch in `DyeApp.tsx`. Footer/hint: "⌘V or P paste".
