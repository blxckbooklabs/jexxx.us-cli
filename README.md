...
**Signed-in vault reads (API-first):** After `jexxxus auth login`, `account_query`, `/account export`, and `export_vault` try **JEXXXUS | API** first (`JEXXXUS_API_URL`, default `https://api.jexxx.us`) with a fresh Clerk JWT, then **fall back** to direct Supabase (RLS + `SUPABASE_ANON_KEY` in `.env`) if the API is down. Cross-user reads require JEXXXUS super-admin credentials plus service-role key—see [SECURITY.md](./SECURITY.md). API server hosts must set `JEXXXUS_ACCOUNT_API=off` to avoid loopback.

**Signed-in vault writes — full CRUD:** `add_contact`/`update_contact`/`delete_contact`, `add_journal_entry`/`update_journal_entry`/`delete_journal_entry`, `manage_contact_event` (NXT logged dates/events—create/update/delete), and `manage_playlist` use the exact same RLS-scoped client as the reads above—`createUserSupabaseClient()` with the signed-in user's own Clerk JWT, never the service-role key. Since BLXCKBOOK/NXT/TV dashboards already subscribe to Realtime on these tables, a write from the CLI shows up live in the dashboard with no refresh. `add_contact` only ever inserts into `api.contacts` (BLXCKBOOK)—a Postgres trigger (`trg_sync_contact_to_vessel`) mirrors the row into `public.vessels` (NXT) automatically in both directions, so a new contact appears on both dashboards without the CLI writing to both tables itself (which would race the trigger). These tools **never** accept `asUserId`—unlike `account_query`, there is no super-admin write-on-behalf-of-another-user path; a signed-in agent session can only ever mutate its own account's rows.
